...
const Badges = ({ loading, badges, match, history }) => (!loading ? (
  <div className="Badges">
    <div className="page-header clearfix">
      <h4 className="pull-left">Badges</h4>
      <Link className="btn btn-success pull-right" to={`${match.url}/new`}>Add Badge</Link>
    </div>
    {badges.length ? <Table responsive>
      ...
      <tbody>
        {badges.map(({ _id, title, createdAt, updatedAt }) => (
          <tr key={_id}>
            <td>{title}</td>
