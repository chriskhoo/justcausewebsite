...
class CharityEditor extends React.Component {
  ...
  handleSubmit() {
    ...
    // get form elements for fields generated by badges
    let checkedBadgesIds = [];
    [...this.form.getElementsByTagName("input")].filter(element=>element.name.split('-')[0]=="badge").filter(badgeBox=>badgeBox.checked).map(badge=>checkedBadgesIds.push(badge.name.split('-')[1]));
    let badges_awarded = [];
    checkedBadgesIds.forEach((_id)=>{
      let badge_awarded = {};
      badge_awarded._id = _id;
      badge_awarded.reason = this.form[`badge-${_id}-reason`].value.trim();
      badges_awarded.push(badge_awarded);
    })

    // get form elements for fields generated by programs
    let charityProgramIds = [];
    let programs = [];
    [...this.form.getElementsByTagName("input")].filter(element=>element.name.split('-')[0]=="program").filter(element=>element.name.split('-')[2]=="name").map(nameElm=>charityProgramIds.push(nameElm.name.split('-')[1]));
    charityProgramIds.forEach((_id)=>{
      prefix = _id.substring(0,4);
      let program = {};
      if(prefix == "temp"){
        program._id = new Date().getTime().toString(36);
      }else{
        program._id = _id;
      }
      program.name = this.form[`program-${_id}-name`].value.trim();
      program.summary = this.form[`program-${_id}-summary`].value.trim();
      program.revenue_model = this.form[`program-${_id}-revenue_model`].value.trim();
      programs.push(program);
    })

    // get form elements for fields generated by governance checklist
    let checkedGoveranceChecklistIds = [];
    [...this.form.getElementsByTagName("input")].filter(element=>element.name.split('-')[0]=="governance_checklist").filter(box=>box.checked).map(boxElement=>checkedGoveranceChecklistIds.push(boxElement.name.split('-')[1]));
    let governance_checklist_ids = [];
    checkedGoveranceChecklistIds.forEach((_id)=>{
      let checklist_awarded = {};
      checklist_awarded._id = _id;
      checklist_awarded.description = g_checks.filter(element=>element._id==_id)[0].description;
      governance_checklist_ids.push(checklist_awarded);
    })

    // get form elements for fields generated by financial checklist
    let checkedFinanceChecklistIds = [];
    [...this.form.getElementsByTagName("input")].filter(element=>element.name.split('-')[0]=="financial_checklist").filter(box=>box.checked).map(boxElement=>checkedFinanceChecklistIds.push(boxElement.name.split('-')[1]));
    let financial_checklist_ids = [];
    checkedFinanceChecklistIds.forEach((_id)=>{
      let checklist_awarded = {};
      checklist_awarded._id = _id;
      checklist_awarded.description = f_checks.filter(element=>element._id==_id)[0].description;
      financial_checklist_ids.push(checklist_awarded);
    })
    ...
    chty.leadership_info={
      ...
      person_1_face: person_1_face.value.trim(),
      ...
      person_2_face: person_2_face.value.trim(),
